//! Canvas Project Feed 集約 WebSocket ハンドラー
//!
...
//! 旧名「Lane」は VP-77 / SP architecture の Lane (= SP 内 Lead/Worker pane) と命名衝突するため、
...
pub struct ProjectFeedInfo {
    /// プロジェクト名
    pub name: String,
    /// Process ポート番号
    pub port: u16,
    /// 接続状態
    pub status: ProjectFeedStatus,
}

/// Lane 接続状態
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ProjectFeedStatus {
    /// Process に接続中
    Connected,
    /// 接続試行中
    Connecting,
...
enum ProjectFeedEvent {
    /// Lane 一覧の更新
    Lanes { lanes: Vec<ProjectFeedInfo> },
    /// Process メッセージ（Lane ラップ）
    LaneMessage {
        lane: String,
        port: u16,
...
enum BridgeMsg {
    ...
    /// Lane 一覧更新
    LanesUpdated(Vec<ProjectFeedInfo>),
    /// Lane 切り替え指示
    SwitchLane { lane: String },
}

/// WebSocket ハンドラー: `/ws/lanes`
pub async fn project_feed_ws_handler(
    ws: WebSocketUpgrade,
    State(state): State<Arc<AppState>>,
) -> impl IntoResponse {
    ws.on_upgrade(move |socket| handle_project_feed_socket(socket, state))
}

/// Canvas クライアントとの WebSocket 接続を管理
async fn handle_project_feed_socket(socket: WebSocket, state: Arc<AppState>) {
    ...
    // 初期 Lane 一覧を送信
    let lanes_event = ProjectFeedEvent::Lanes {
    ...
    let cmd_task = tokio::spawn(async move {
        while let Some(json) = cmd_rx.recv().await {
            // switch_lane は専用 BridgeMsg で送信（ProjectFeedEvent 正規パス）
        ...
        while let Some(msg) = bridge_rx.recv().await {
            let event = match msg {
                ...
                BridgeMsg::StatusChanged {
                    ...
                    if matches!(status, ProjectFeedStatus::Disconnected) {
                        ports_for_send.lock().await.remove(&port);
                        tracing::debug!("Lane {} (port {}) を追跡セットから除去", lane, port);
                    }
                    ProjectFeedEvent::ProjectFeedStatusChanged {
                        lane: lane.clone(),
                        port,
                        status: status.clone(),
                    }
                ...
                BridgeMsg::LanesUpdated(lanes) => ProjectFeedEvent::Lanes { lanes },
                BridgeMsg::SwitchLane { lane } => ProjectFeedEvent::SwitchLane { lane },
            };
        ...
        while let Some(Ok(msg)) = ws_receiver.next().await {
            match msg {
                Message::Text(text) => {
                    ...
                    // ProjectFeedCommand を処理
                    if let Ok(cmd) = serde_json::from_str::<ProjectFeedCommand>(&text) {
                        match cmd {
                            ProjectFeedCommand::Refresh => {
...
async fn discover_and_connect(
    ...
    for proc in &running_processes {
        ...
        if proc.port == state.port {
            ...
            lanes.push(ProjectFeedInfo {
                name: proc.project_name.clone(),
                port: proc.port,
                status: ProjectFeedStatus::Connected,
            });
            continue;
        }
        ...
        let tx = bridge_tx.clone();
        tokio::spawn(async move {
            spawn_project_feed_bridge(lane_name, port, tx).await;
        });

        lanes.push(ProjectFeedInfo {
            name: proc.project_name.clone(),
            port: proc.port,
            status: ProjectFeedStatus::Connecting,
        });
    }
...
async fn spawn_project_feed_bridge(lane: String, port: u16, bridge_tx: mpsc::Sender<BridgeMsg>) {
        ...
        Err(e) => {
            ...
            let _ = bridge_tx
                .send(BridgeMsg::StatusChanged {
                    lane,
                    port,
                    status: ProjectFeedStatus::Disconnected,
                })
                .await;
            return;
        }
