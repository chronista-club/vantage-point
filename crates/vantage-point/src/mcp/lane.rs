...
pub struct AddSubParams {
    ...
    #[schemars(
        description = "この lane の claude model alias (省略可、例: 'opus' / 'sonnet' / 'haiku' / 'claude-fable-5-1')。task 難度に合わせて指定する (機械的作業=sonnet / 中核設計=opus 等)。tui spawn・respawn・gui engine が共有。省略時は config の default-lane-model (既定 Opus)。"
    )]
...
pub struct FlowHandoffParams {
    ...
    /// Optional claude model alias for this lane (co-evolution #1)
    #[schemars(
        description = "worker の claude model alias (省略可、例: 'opus' / 'sonnet' / 'haiku' / 'claude-fable-5-1')。task 難度に合わせて指定 (機械的=sonnet / 中核設計=opus)。省略時は config の default-lane-model (既定 Opus)。"
    )]
    #[serde(default)]
    pub model: Option<String>,
