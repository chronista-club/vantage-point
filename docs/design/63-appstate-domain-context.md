...
> **Status**: **9-2 完了（2026-09-11）— 段階 1 / 2 / 3 すべて着地** — PR-A〜5（#1099 / #1101〜#1110、起票 #1098 を含めて 12 commit）が全部 nightly に着地。`RepoState`（14 field）/ `DaemonState`（25 field）の 2 型。段階 2（`BoardContext` #1112 / `EditorContext` #1113）完了。段階 3（停止責任）は PR-S3a（runner）/ PR-S3b（repo の常駐 task）/ PR-S3c（daemon の常駐 task）/ PR-S3d（Editor pending の guard 化）で**段階 3 完了 = 9-2 完了**（S3b / S3c は `ActorRegistry::stop_all`、S3a は `process_runner::stop_all` で終了を確認、S3d は §8 穴 ②）。残す follow-up は §8（runner の cut 判断 / PR-6 任意 / test 未固定の spawn 経路）。file 名は「`AppState` を分ける」という設計の主題なので据え置き（doc 44 が `world` を残すのと同じ）
...
| **3** | 所有と寿命（停止責任） | **停止を要求した仕事の終わりまで確認できる** | PR-S3a: runner（`process_registry`）を repo stop で止めて終了を確認 ✅ / PR-S3b: repo が spawn する常駐 task 5 本を `ActorRegistry` に預け、`shutdown_repo` が `actor_registry::stop_all` で終了を確認 ✅ / PR-S3c: daemon の常駐 task 8 本を同じ registry に預け、`run_daemon` の停止末尾で `stop_all` ✅ / PR-S3d: §8 穴 ②（Editor pending）を `PendingGuard` で構造化 ✅ — 段階 3 完了 |
...
| **3** ✅ | daemon 役 `AppState` を**構築ごと**削除（`repo/server.rs:772` の全 field — 現 HEAD で 29、**PR-2c 後は 19** + `hub` / `topic_router` local）+ **wire 系 4 field も同時削除** | 不変条件 3 を守る。本文の変更は `repo/server.rs` + `state.rs`（wire 4 field）に限る。**`--no-default-features` でも check を 1 回** |
...
必須 field を持つ名前付き引数用 struct（`DaemonAssembly`）で入れた。**`Default` で穴を埋める仕組みには戻さない** — `Option` は「DB 接続失敗で無い」3 つ（`vpdb` / `wiremsg_store` / `delegation_store`）だけ。`Default` / `new()` 自体は test のために残した（`daemon/process.rs::run_daemon` は呼び手が無く PR-3 で削除）。bind・PID 書き込み・外部サービス起動の順序は動かしていない（不変条件 3）。
...
### 段階 3 で拾う既知の穴 2 件（① は PR-S3a、② は PR-S3d で解決）
...
- ~~**② Editor の呼び出し中断で pending が残る**~~ → **PR-S3d で構造的に閉じた**（2026-09-11）。Codex の probe（`.vp/reports/probes/editor-cancel/`、`pending_after_future_drop_and_3_1s=1` / `pending_after_late_response=0` / `pending_after_normal_timeout=0`）が指した「往路の future が timeout の前に drop されると entry が残る」を、**本番経路で起きるか先に実測**した: 往路に届く forward 経路は daemon の repo-proxy channel handler（`daemon/server.rs` の `register_channel("repo-proxy")`）と gui surface channel の 2 本で、どちらも request を inline で await し `select!` / `timeout` / spawn で包んでいない。unison は stream ごとに handler を `tokio::spawn` し切断時も abort しない（abort するのは datagram task だけ、`club-unison-2.0.0/src/network/dispatch.rs`）。abort されうる forward の呼び手（delivery / delegation の常駐 task、PR-S3c の `stop_all` の射程）は editor の method を送らない — つまり「起きない」は経路の本数ではなく**送る method に依存する**前提で、editor を別経路に載せたら破れる。MCP client の 5 秒 timeout は bridge の 3 秒より長く、client が先に諦めても server の future は走り切る。daemon 停止の `daemon_handle.abort()` は accept loop だけで per-stream task には届かず、その後は process が終わる。**= 今の本番経路ではこの drop は起きない。** それでも直したのは、掃除が「timeout 分岐の 1 行」という散文契約で、呼び手が `timeout` で包んだ瞬間に壊れる形だったから。`PendingGuard`（`Drop` で remove）に変え、`EditorPending` は `std::sync::Mutex` に（`Drop` は同期。lock を持ったまま await する箇所は無い）。test は probe をそのまま unit test にした（future を drop した直後に pending が空）。
...
→ **PR-S3b（2026-09-11）で repo 側は実装済**。`RepoTasks` という新しい型は作らず、`ActorRegistry` に `spawn_task`（mailbox を持たない loop 用、`ActorKind::Task`）/ `close_and_take_tasks` / `stop_all` を足した。`start_repo` が spawn する 5 本（TopicRouter bridge / lane-spawn actor / lanes publish / lifecycle monitor / idle sweep）は全部 registry 経由になり、`shutdown_repo` が runner の次に `stop_all` で終了を待つ（task ごと 8 秒で見切って `abort`）。`LaneSpawnActor` は Cmd ごとの子 task を `JoinSet` で持ち、どちらの終了経路でも子を待ち切ってから返る（契約 ③。親を `abort` すると `JoinSet` の drop で子も落ちる）。**daemon 側は PR-S3c（2026-09-11）で接続**: `run_daemon` の常駐 task 8 本（update-check / creo-actions-poll / DeliveryActor / delegation-reconcile / hub-federation / autostart-repos / lane-watcher / live-select-bridge）を同じ `actor_registry`（= `DaemonState.actor_registry`）に `spawn_task` / `spawn_service` で預け、停止末尾は `daemon_handle.abort()` → **`stop_all`** → `shutdown_all` → capability shutdown → `remove_pid_file`。`stop_all` を `shutdown_all` より前に置くのは、lane watcher が drain 中の repo に `lane_delete` を撃たないため、そして autostart が起動途中の repo を drain の後に差し込まないため（先に `RepoRuntimes::close` で受付を閉じる）。予算は 2 秒（`stop_all_with`）。`run_daemon` の本文に `tokio::spawn` で残るのは `daemon_handle`（明示 `abort()`）と signal handler（signal を待つだけで cancel では抜けない。Unix の `vp daemon stop` は SIGTERM なので handler は完走するが、`/api/shutdown` 経路 — Windows の `stop_daemon` / 手 POST — では pending のまま process 終了で消える。registry に載せるとその経路で timeout を食うので載せない）の 2 本。task の中からさらに spawn する子（`DeliveryActor` の `spawn_bg_dispatch` が起こす `claude` 子プロセス等）は registry の射程外で、これは S3b の `LaneSpawnActor` と違い idempotent なので追わない。update-check / creo-actions-poll は request 中（reqwest 10 秒）は cancel を見ないので、その瞬間に停止が来ると timeout → abort になる（abort で安全に落ちる。tokio の `RwLock` は poison しない）。`delegation::spawn_reconcile_loop` は spawn せず future を返す `reconcile_loop` に。⚠️ **repo 側の「5 本が registry 経由」は test で固定していない**（`start_repo` を叩く test が crate に無く、1 本を素の `tokio::spawn` に戻す mutation は緑のまま通る）。固定した契約は「registry に載った task は `shutdown_repo` が待つ」（server.rs の test）と「`LaneSpawnActor` は子を待ち、子は cancel で降りる」（spawn_actor.rs の test）の 2 つ。
...
| `editor_pending` | `Arc<Mutex<HashMap<_, oneshot::Sender<_>>>>` | `Default` | 自分 | 独立 | prod 3 / test 2、1 file | 登録側 timeout remove / 解決側 remove（idempotent）→ PR-S3d で登録側は `PendingGuard` の `Drop`、`Mutex` は `std::sync` |
...
- 2026-09-11: **段階 3 PR-S3c（daemon の常駐 task の停止責任）。** S3b と同型。`run_daemon` の `tokio::spawn` 7 箇所を `spawn_task` 経由に（DeliveryActor は S3b 以前から `spawn_service`、合わせて 8 本。registry は `DaemonState` と同じ Arc、scope は `LayerScope::Machine`）。停止末尾で `RepoRuntimes::close`（受付を閉じる）→ `stop_all_with`（予算 2 秒 — Unix の `vp daemon stop` は SIGTERM の 500ms 後に SIGKILL するので repo / db の後始末を kill 期限の外へ押し出さない）→ `shutdown_all`。`delegation::spawn_reconcile_loop` → `async fn reconcile_loop`。signal handler と `daemon_handle` は載せない（§8）。Moody Blues 3 件を反映: autostart に token を渡して repo ごとの境目で降りる（token 無しだと `stop_all` の見切り abort が `start_repo` の途中で future を drop し、`RepoRuntimes.closing` の doc が防ぎたいと書いている孤児化そのものになる）/ hub federation の connect（5 回試行）と register を cancel と競わせる（hub 不達のまま停止すると毎回 timeout を食っていた）/ `close` を `stop_all` の前に。「lane watcher を repo より先に止める」は書いていなかった理由の方が強い — FSEvents から drain 中の repo に `lane_delete` を撃たないため。「8 本が registry 経由」は S3b の 5 本と同じく test 未固定（`run_daemon` を叩く test が無い）。
- 2026-09-11: **段階 3 PR-S3d（Editor pending の guard 化）— 段階 3 完了、9-2 完了。** 穴 ② を「本番経路で起きるか」から測った（起きない: unison は per-stream handler を abort しない、repo-proxy loop は inline await、client timeout 5 秒 > bridge 3 秒）。直したのは散文契約を構造に変えるため: `PendingGuard` の `Drop` で remove、`EditorPending` は `std::sync::Mutex`。test +1（probe の再現）、mutation（guard を外して timeout 分岐の remove に戻す）で赤。
