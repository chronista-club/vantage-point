...
pub async fn delete_lane_orchestrated(
    ...
    state.reconcile_terminal_pumps(&addr.to_string()).await;
...
pub(crate) async fn emit_lane_update(state: &RepoState, addr: &LaneAddress) {
    ...
    super::enrich::refresh_engine_session_id(&mut info);
    // 活動時刻は refresh と対（apply_session_activity の doc ⚠️ — 供給点差は #683 地形）。
    super::enrich::apply_session_activity(&mut info, &activity);
    if let Err(e) = state
        .system_event_tx
...
async fn converge_lane(
    ...
    for attempt in 0..RESTART_MAX_ATTEMPTS {
        let r = state.reconcile_lane(&addr).await;
        if r.failed == 0 {
            let pid = state
                .lane_pool
                .read()
