...
impl Capability for AgentCapability {
    ...
    fn diagnose(&self) -> crate::capability::DiagnosticReport {
        ...
        // `AppState.agent_msgbox_main` も撤去、 全 msg routing は `msgbox_store`
    ...
    async fn initialize(&mut self, ctx: &CapabilityContext) -> CapabilityResult<()> {
        ...
        // 旧: ctx.msgbox().recv() loop で agent#main の msg を消費 → EventBus emit
        // 中継: agent box の owner は AppState.agent_msgbox_main に移管
