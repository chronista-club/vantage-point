//! lane 単位の stand（engine 種別）永続 — performer の stand を repo 再起動をまたいで保つ
//!
//! ## 背景（bug mem_1Cd4M7i5Enp3HHMLVYayRe、2026-07-16）
//!
//! performer の stand は従来 in-memory（LanePool の LaneInfo.stand）にしか無く、repo 再起動後の
//! boot bootstrap（server.rs）は disk scan した全 performer を config の `default_stand`
//! （= echoes）で spawn していた。「performer の stand は repo 再起動をまたいで永続しない」の
...
//! - **読み手**: repo boot bootstrap（server.rs）が SpawnLane Cmd の stand に使う
//!   （記録不在 = 旧 lane / 手動 `vp lane new` は従来どおり config default に fallback）
//! - 置き場: `vp_state_dir()/lane_stands/<repo>__<lane>`（1 lane 1 file 1 行 = stand 名）
...
pub fn record_in(base: &Path, repo: &str, lane: &str, stand: &str) -> std::io::Result<()> {
    STORE.record_in(base, repo, lane, stand)
}
...
pub fn record(repo: &str, lane: &str, stand: &str) -> std::io::Result<()> {
    record_in(&crate::config::vp_state_dir(), repo, lane, stand)
}
