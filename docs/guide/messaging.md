...
`WiremsgStore`（`wiremsg_store.rs:185`）は TheWorld daemon の in-process DB（embedded SurrealDB）を持つ唯一の writer。SP の wire ハンドラも CLI も、`world_wire::call(path, payload)`（`world_wire.rs:124`）で `/api/wire/*` という論理 path を投げ、それが QUIC "wire" channel の method（`wire/send` 等）として TheWorld に届く。SP は **1 プロセス 1 本の永続 QUIC 接続**を再利用する（per-call 新造は fd leak を起こし、過去に RLIMIT_NOFILE 枯渇で mesh が全滅した経緯がある — `world_wire.rs` module doc）。
...
- 消費側: TheWorld の `dispatch_wire("send")` が `body.category == "command"` を見て delivery loop を即 wake する（`routes/wire.rs:361`）。
...
### 2.2 `LaneInfo.flow_state` 投影経路（SP → World → vp-app）

```
performer の wire 活動
  → WiremsgStore（TheWorld in-process）
  → [SP]   build_lanes_snapshot（flow_state = None のまま）
             discovery "registry" channel: register / lanes/add|remove|update（heartbeat 15s）
  → [World] lane_registry: HashMap<path_key, Vec<LaneInfo>>   ← ここまで flow_state = None
             send_lanes_snapshot → enrich_lanes_flow_state → derive_flow_state   ← ここで付与
             "lanes" channel で send_event("snapshot", LanesSnapshot)
  → [vp-app] "lanes" channel を open（World :32000 に接続、SP 直結ではない）
             laneConnector(flow_state) → sidebar connector 描画
```

投影の実装事実:

- **`LaneInfo.flow_state`（`lanes_state.rs:324`）は `Option<FlowState>`**。SP / lane_registry / db では**常に `None`**（「derive できるものは store しない」原則）。付与するのは World だけ。
...
- **"lanes" channel は unison/QUIC channel**（`register_channel("lanes")`、`daemon/server.rs:1218`）。WebSocket でも SSE でもない。vp-app は **SP ではなく World :32000 の集約 channel** に繋ぐ（`vp-app/src/app.rs` の lanes subscription、stall timeout 12s）。
...
## 3. federation（cross-world / cross-PC）

別マシン（別 world）の agent へ wire を届ける仕組み。address 文法（`actor@world/project/lane`）は [`wire-address-usage.md`](./wire-address-usage.md) を、設計背景は同 spec を参照。ここでは **現状動く挙動**を実装から記す。
...
### 3.1 world identity と到達性

| 概念 | 実体 |
|---|---|
| `wld_id` | home-World の位置独立な安定 id（`world/world_id.rs`、`wld_` + base58(UUIDv7)、opaque な routing key、home-World に 1 個、db 永続）。direct の SNI にも使う |
| handle | world の表示名。**OS hostname から決まる**（`resolve_handle(None)`、`hub_client.rs:220`。override → hostname → fallback `"vp-world"`） |
| endpoints | direct 到達候補。**IPv6 GUA（`2000::/3`）のみ**を advertise（`world/endpoint.rs`、link-local / ULA / loopback は除外）。connect-trick で OS が選んだ source GUA を読む。**tailnet 非依存**。空なら relay floor に委ねる |
...
hub と話すのは **TheWorld daemon のみ**。CLI / SP は daemon の wire channel 経由で federation を叩く（SSOT）。

### 3.2 register（自 world の登録）
...
### 3.3 discover（相手 world の lane 列挙）

```bash
vp wire discover --world <handle>     # 例: vp wire discover --world taro-box
```

- 宛先を知らないとき、相手 world の lane 一覧を relay 上の request-response で列挙する（在庫確認）。
...
### 3.4 send（cross-world 送信）

```bash
vp wire send --world <handle> --to <logical addr> --body "..."
# 例: vp wire send --world taro-box --to agent@nostos/main --body "hi"
```

- `--world` を付けると path が `/api/wire/federate` になり、TheWorld が hub relay 経由で遠方 world へ送る。**`--to` は「その world 内部の logical address」**として解釈される（自 world の address ではない）。
...
### 3.7 ⚠ 罠: `--world` 省略で「同名 project のローカル宛」に silent 成功

**最重要の落とし穴。** `--world` は `Option` で、`None` のとき path が `/api/wire/federate` ではなく `/api/wire/send`（ローカル中央 store）に落ちる（`commands/wire.rs:704`）:
...
let path = if let Some(remote) = world {
    ...
    "/api/wire/send"          // ← --world 省略はここ。エラーにならない
};
```

- 遠方 world へ送るつもりで `--world` を書き忘れると、**エラーにならず**、`--to` が**ローカルの同名 project 宛**として解釈されて **silent に成功**する。宛先 world の実在検証は send 経路に無い。
- 対策候補（未実装、issue 扱い）: origin world を継承する federation の `--reply-to` 相当があれば、返信で world を書き忘れる事故を防げる。

---

## 4. 入口一覧表（MCP ⇄ CLI ⇄ 内部 channel method）

> ⚠ **「HTTP」列は公開 HTTP API ではない**。かつての `/api/wire/*` axum route は撤去済（`routes/health.rs:49`）。今この文字列は `world_wire::call` に渡す**論理 path** で、`/api/` を剥いだ残り（`wire/send` 等）が **unison QUIC "wire" channel の method** になる。3 者はすべて同じ下層 `WiremsgStore`（TheWorld）に収束する。
...
- **CLI は World 直結**（`world_wire::call`）で、address は qualified 前提（`from` の default は `"vp-cli"`）。
- **category default の非対称**（§1.4）: MCP `wire_send` は `command` を注入、CLI `vp wire send` は注入しない。
- 共通下層: writer = `WiremsgStore`（TheWorld、local_seq を AtomicU64 採番）、transport 集約点 = `world_wire::call`、dispatch 分岐 = `handle_wire_channel`（`wire/` vs `delegation/`）。
