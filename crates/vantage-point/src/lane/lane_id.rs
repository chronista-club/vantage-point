...
//! 動こうと project が rename されようと、 この id は変わらない (= 発端バグの
//! path=identity を断つ種)。
//!
...
//!   cc_session (`<project>__<lane>`) と同じ命名規則。
//!
//! 設計は [`crate::lane::cc_session`] を mirror (純関数 `*_in(base)` + 本番 wrapper)。

use std::path::{Path, PathBuf};

use crate::process::lanes_state::LaneId;
...
pub fn id_file_in(base: &Path, project: &str, lane: &str) -> PathBuf {
    base.join("lane_ids")
        .join(format!("{}__{}", sanitize(project), sanitize(lane)))
}

/// (project, lane) に対応する安定 id を取得する。
///
...
pub fn load_or_create_in(base: &Path, project: &str, lane: &str) -> LaneId {
    let path = id_file_in(base, project, lane);
...
pub fn load_or_create(project: &str, lane: &str) -> LaneId {
