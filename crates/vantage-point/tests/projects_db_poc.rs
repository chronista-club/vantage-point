...
// =====================================================================
// PR-C: replace_all_projects (persist_projects の DB 全置換セマンティクス)
// =====================================================================
...
async fn b1a_replace_all_removes_stale_and_reorders() {
    ...
    db.import_projects(&three).await.unwrap();
    ...
    db.replace_all_projects(&two).await.unwrap();
    let rows = db.list_projects().await.unwrap();
...
/// B1b: replace_all(&[]) で projects テーブルが空になる (= 全 project 削除の DB 反映)。
#[tokio::test]
async fn b1b_replace_all_empty_clears() {
    ...
    db.replace_all_projects(&[]).await.unwrap();
    assert_eq!(
        db.list_projects().await.unwrap().len(),
        0,
        "空 Vec で全消し"
    );
...
/// B1c: path 重複を含む entries → UNIQUE index (idx_projects_path) で後勝ち 1 件に集約。
#[tokio::test]
async fn b1c_replace_all_dup_path_last_wins() {
    ...
    db.replace_all_projects(&dups).await.unwrap();
    let rows = db.list_projects().await.unwrap();
...
/// (= persist_projects の「DB 全置換 + kdl ミラー」往復が情報を失わない)。
#[tokio::test]
async fn b2_replace_all_roundtrip_via_kdl() {
    ...
    db.import_projects(&sample()).await.unwrap();
    let kdl = ProjectsFile {
        projects: db.export_projects().await.unwrap(),
    }
    .to_kdl()
    .unwrap();
    ...
    let back = ProjectsFile::from_kdl(&kdl).unwrap();
    db.replace_all_projects(&back.projects).await.unwrap();

    let again = db.export_projects().await.unwrap();
