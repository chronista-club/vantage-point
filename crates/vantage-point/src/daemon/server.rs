...
//!
//! daemon の live channel（daemon-repo / events / wire / registry / device 等）を提供。
//! repo の自己登録を受け付け、process lifecycle / wire / event log を中継する。
...
use crate::capability::{RepoPresenceState, RunningRepo};

/// repo control channel registry（key = `path_key`、 value = repo の live control channel）。
///
/// daemon server の repo-proxy / registry handler が populate し、 canvas/terminal_write の
/// reverse-routing に使う。 tmux decoupling PR1: Daemon-side の nudge loop（delivery_actor /
/// delegation reconcile）も同一 map を引いて `lane_nudge` を所有 repo に forward する（SSOT はここ）。
...
pub struct DaemonState {
    ...
    /// repo情報（RepoManagerCapability と共有、状態更新用）
    ...
    pub process_presence: Option<Arc<RwLock<HashMap<String, RepoPresenceState>>>>,
    /// VP-154 PR-2: Process lifecycle event broadcast bus (= "daemon-repo" channel の data plane)
    ///
    /// registry channel handler が repo register/unregister を受信したタイミングで `send` し、
    /// "daemon-repo" subscribe handler の broadcast::Receiver が pump して client に
    /// `send_event` で push する経路。 capacity 64 = repo 同時 register が短時間に集中しても
    /// drop しない buffer (= 既存 system_event_tx と同サイズ)。
    pub process_lifecycle_tx: tokio::sync::broadcast::Sender<ProcessLifecycleEvent>,
    /// L0 SP-portless (lanes slice): lane_registry が変化した repo の path_key を載せる
    ...
    /// per-repo "lanes" channel の subscriber がこれを購読して当該 repo の現 snapshot を
    ...
    ///
    /// 各 repo が "canvas-ingest" channel で board の RepoMessage を push し、 daemon は
    /// それを repo の TopicRouter に `route()` する。 vp-app 向け "canvas" channel はこの
    /// TopicRouter を `subscribe("repo/board/#")` して retained 初期配信 + live delta を
    /// 配る。 repo の "canvas" channel (`repo/unison_server.rs`) と **同じ TopicRouter 型を再利用**
    /// することで、 retained/delta/atomicity を daemon 側で再実装せず委譲する (lanes の lane_registry
    /// に相当する canvas 版の per-repo store)。 repo_path (path_key) → TopicRouter。
    /// 初出 repo は ingest / subscribe のどちらか早い方が get-or-create する。
    ///
    /// ライフサイクル: lane_registry と同じく **repo 切断では drop しない** (= retained canvas を
    /// repo restart 越しに保持 = 「前回の続き」)。 entry の回収は repo remove (namespace ごと撤去)
    /// のタイミングが正で、 現状は未実装の follow-up (control_channels が repo 切断で remove するのと
    ...
    ///
    /// doc 44 P1 (fold-in) 以前: 各 repo が起動時に "control" channel で daemon に outbound 接続し、
    /// daemon はその `UnisonChannel` を path_key で保持していた。"repo-proxy" 経由で来た外部
    ...
    pub daemon_cap: Option<Arc<RwLock<crate::capability::RepoManagerCapability>>>,
    /// doc 24 §10 Phase 2: lane descriptor の durable 永続先 (daemon-canonical 化)。
    ///
    /// registry channel handler が repo push (register snapshot / lanes diff) を受けた時、
    /// in-memory `lane_registry` への反映と並行して db に永続する。 これにより repo disconnect /
...
// =========================================================================
// daemon control Channel ハンドラー（repos mutation: CLI → Daemon 直接 Unison）
// =========================================================================

/// "daemon-control" channel の method を `RepoManagerCapability` に dispatch する。
///
...
pub(crate) async fn handle_daemon_control(
    ...
    // 内部 mutation は RepoManagerCapability の Arc<RwLock> field で直列化される。
    match method {
        "repos/list" => {
            let list = daemon_cap.read().await.list_repos().await;
        ...
        // doc 45 段 1: HTTP `POST /api/daemon/repos/update` の Unison 版。
        // rename と set_enabled をまとめて適用する部分更新（vp-app の編集 dialog が使う形）。
        // 個別の `repos/rename` / `repos/set_enabled` と実体は同じで、こちらは
        // 「1 往復で両方直す」ための合成入口（HTTP と同じ `apply_repo_update` を共有）。
        "repos/update" => {
            let path = payload["path"]
                .as_str()
                .ok_or_else(|| "path is required".to_string())?;
        ...
        // repos.kdl / db から ghost repo（dir が実在しない登録）を除去する。
        ...
        // doc 45 段 1: HTTP `POST /api/daemon/repos/reload` の Unison 版。
        // repos.kdl を読み直して in-memory repos に反映する（VP-189）。
        // CLI が repos.kdl を書き換えた後に稼働 daemon の乖離を解消する best-effort 通知。
        "repos/reload" => {
        ...
        // doc 44 P1 (fold-in): 単一 repo の lifecycle 制御。旧 `vp sp start/stop` の後継で、
        // 名詞を「repo（プロセス）」から「repo」へ移した（D2: repo はプロセスではなく
        // daemon が抱える map のエントリ）。旧 `vp sp start` は repo を daemon の外で
        ...
        "repos/start" => {
            ...
            Ok(serde_json::json!({
                "status": "started",
                "name": proc.repo_name,
                "path": proc.repo_path.to_string_lossy(),
            }))
        }
        "repos/stop" => {
            let name = payload["name"]
        ...
        // repo の PointView を開く（未起動なら内部で start_process する）。
        ...
        // repo の active lane (presence、Model Q) を daemon-canonical に設定する。
        "lanes/set_active" => {
            let path = payload["path"]
                .as_str()
                .ok_or_else(|| "path is required".to_string())?;
        ...
        // doc 44 §7.5: Repo Host の帳簿（見送りの記録）を CLI から触る 3 面。
        //
...
///
/// `running_repos`（port/name の SSOT）と `lane_registry` を repo ごとに join し、
/// `daemon_cap` の repo_order で安定ソートした repos 配列（`Vec<serde_json::Value>`）を返す。
/// "daemon-repo" channel の `list_all_lanes` handler と、DeviceRegistry 常駐 ROTO loop の
/// `InProcessLaneSource`（in-process 直読み）が **同一ロジックを共有**することで、
/// CLI（QUIC 経由）と daemon（直読み）で lane 並びが完全一致する（doc 23 の重複回避）。
///
/// ロック順序: running_repos → lane_registry（register と同順、deadlock 回避）。
#[allow(clippy::type_complexity)]
pub(crate) async fn build_node_lanes(
    ...
    // ロック順序統一: running_repos → lane_registry（register と同順）。
    let mut entries: Vec<(usize, serde_json::Value)> = Vec::new();
    {
        ...
        for (key, p) in procs.iter() {
            let lanes = lanes_map
                ...
                .cloned()
                .unwrap_or_default();
            // repo_order 内の位置。未登録は末尾（usize::MAX）。
            let idx = order
                .iter()
                .position(|n| n == &p.repo_name)
                .unwrap_or(usize::MAX);
            entries.push((
                idx,
                serde_json::json!({
                    "repo_name": p.repo_name,
                    "repo_path": p.repo_path.to_string_lossy(),
                    "port": p.port,
                    "lanes": lanes,
                }),
            ));
    ...
    // repo_order 順に整列（= sidebar 順）。同 idx は安定ソートで維持。
    entries.sort_by_key(|(idx, _)| *idx);
    entries.into_iter().map(|(_, v)| v).collect()
}

/// L0 SP-portless (lanes slice): per-repo "lanes" channel に現 lane snapshot を 1 回 push する。
///
/// `lane_registry[path_key]` の現値を `RepoMessage::LanesSnapshot` に包み、 repo "lanes" channel と
...
async fn send_lanes_snapshot(
    ...
    if let Some(store) = wiremsg_store {
        // wire address の `<repo>` は registry 登録名 (= repo の config 登録名) が SSOT
        // (`resolve::repo_name_from_path` と同値)。 repo 未接続の窓は lane address の
        // repo (basename 由来) で代用する (両者は通常一致)。
        let repo_name = match running_repos {
            Some(rp) => rp.read().await.get(path_key).map(|p| p.repo_name.clone()),
            None => None,
        }
        .or_else(|| lanes.first().map(|l| l.address.repo.clone()));
        if let Some(repo_name) = repo_name {
            enrich_lanes_flow_state(&mut lanes, store, &repo_name).await;
        }
    }
    // doc 44 D4: 開発起点を帳簿から解決して添える。repo runtime 側の publish
    ...
    let snapshot = crate::protocol::RepoMessage::LanesSnapshot {
        lanes,
        origin: Some(origin),
    };
...
async fn enrich_lanes_flow_state(
    ...
    for lane in lanes.iter_mut() {
        ...
        let agent_addr = format!("agent@{}/{}", repo_name, lane.address.name);
        let latest = store
...
/// FSM 投影: wire payload から関与 repo 名を抽出する (純関数)。
///
/// `wire/send` (from + to[]) / `wire/ack` (agent) の payload に現れる agent address
/// (`<actor>@<repo>[/<lane>]`) から `<repo>` を集める。 flow_state は wire 活動で
/// 変わるため、 send/ack 成功後にこれらの repo の "lanes" subscriber へ再 push を促す。
fn collect_wire_repos(payload: &serde_json::Value) -> Vec<String> {
...
/// FSM 投影: wire 活動 (send/ack) 後に、 関与 repo の "lanes" subscriber へ snapshot
/// 再 push を促す。 flow_state は送信時 derive のため wire 活動がそのまま sidebar の
/// 更新トリガになる (= event-driven、 polling 無し)。 未登録 repo は黙って skip (best-effort)。
async fn notify_lane_change_for_repos(state: &DaemonState, repos: &[String]) {
    let Some(rp) = &state.running_repos else {
        return;
    };
    if repos.is_empty() {
        return;
    ...
    for (path_key, proc) in procs.iter() {
        if repos.iter().any(|p| p == &proc.repo_name) {
...
/// L0 SP-portless (canvas slice): repo の Canvas TopicRouter を get-or-create する。
///
/// "canvas-ingest" (repo push) と "canvas" (vp-app subscribe) のどちらが先でも、 同じ repo の
/// TopicRouter を共有する (= repo push が router に route し、 vp-app subscribe が同 router を購読)。
///
/// S2 (doc 27 §4.1): router 新規作成時に terminal demand hook を登録する。 surface が
/// `repo/terminal/data/{lane}/out` を購読した瞬間 (0→1) / 最後に離れた瞬間 (1→0) に、
/// 当該 repo の control channel を逆用して `terminal_demand_start/stop {lane}` を撃つ
/// (= 購読者が居る間だけ repo pump を回す demand-driven production)。
#[allow(clippy::type_complexity)]
async fn canvas_router_for(
    canvas_routers: &Arc<RwLock<HashMap<String, Arc<crate::repo::topic_router::TopicRouter>>>>,
    ...
    if let Some(state) = control_channels.get(path_key).await {
        ...
        tracing::info!(
            "canvas router を repo の実 router に結線 (key={})",
            path_key
        );
    ...
    // fast path: 既存 router を read lock で取得（repo 未起動時の placeholder 経路）
...
/// repo に撃つ。 cb は sync で呼ばれるため、 reverse-route (async I/O) は `tokio::spawn` に逃がす。
///
...
fn register_lane_demand(
    ...
    router.register_demand(pattern, move |topic, _added| {
        // topic = `repo/<x>/data/{lanekey}/<y>` → lane address を復元
...
/// L0 SP-portless: repo 単位 channel 共通の subscribe handshake を待つ。
///
/// client (repo canvas-ingest / repo control / vp-app lanes・canvas / MCP repo-proxy) は接続後に
/// `request("subscribe", {repo_path})` を送る。 repo_path を path_key に正規化して返す。
/// canvas / lanes / control / repo-proxy の 4 系統が同一 handshake protocol を共有する
...
/// この pattern で指定する (例: terminal surface は `repo/terminal/data/{lane}/out`)。
...
async fn recv_subscribe_handshake_with_pattern(
    ...
    loop {
        ...
        let pattern = payload
            ...
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        let path_key = crate::capability::normalize_path_key(std::path::Path::new(repo_path));
        let _ = channel
...
/// L0 SP-portless: 外部 client の process method を当該 repo の control channel を逆用して forward する。
///
/// "repo-proxy" channel (MCP/CLI) と bidirectional "canvas" channel の upstream request
/// (S3 terminal_write/terminal_resize) が共有する。 repo 未接続 / forward 失敗は error JSON で
...
pub(crate) async fn forward_to_sp_control(
    ...
    // doc 44 P1 (fold-in): 旧実装は path_key で repo の QUIC control channel を逆引きし
    // request を投げていた。repo プロセスが daemon に畳み込まれた今、同じ dispatch
    // (`dispatch_repo_method`) を **同一プロセス内で直接呼ぶ**。
    // これで reverse-route の取りこぼし (repo 未接続の無言破棄 / refire の空振りレース /
...
async fn handle_wire_channel(
    ...
    // cc_session pointer を動かした時の変化通知。repo は portless で hook は Daemon しか
    // 知らないため、ここで repo 名 → path_key を lane_registry から逆引きし、当該 repo の
    // control channel に `lane_session_changed` を forward する。repo が真値を re-enrich して
    // `Diff::Update` を push → 本 daemon の "lanes/update" 受信 → registry replace +
    // lanes snapshot 再 push、という既存経路に乗る（daemon は routing のみ、真実源は repo）。
    // repo 不在 / 未接続は Err（hook 側は fail-open で握る）。
    if method == "lane/session-changed" {
        let repo = payload
            .get("repo")
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
            .ok_or_else(|| "lane/session-changed: 'repo' (repo 名) required".to_string())?;
        let label = payload
            .get("lane")
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
        ...
        let path_key = {
            ...
            registry
                .iter()
                .find(|(_k, lanes)| lanes.iter().any(|l| l.address.repo == repo))
                .map(|(k, _)| k.clone())
        }
    ...
    if let Some(sub) = method.strip_prefix("wire/") {
        ...
        let store = state.wiremsg_store.as_ref().ok_or_else(|| {
            "wire store not initialized (daemon DB 接続失敗 or repo mode)".to_string()
        })?;
        let notifier = state
            .wire_notifier
            .as_ref()
        ...
        // FSM 投影: send/ack は flow_state を変え得るので、 関与 repo を dispatch 前に
        // 控えておき (payload は dispatch に move される)、 成功後に lanes 再 push を促す。
        let wire_repos = if matches!(sub, "send" | "ack") {
            collect_wire_repos(&payload)
        } else {
            Vec::new()
        };
        let result =
...
pub async fn start_daemon_server(state: Arc<DaemonState>, port: u16) {
    ...
    // starter は registry + events のみ。repo-proxy は subscribe handshake 前提で無改造
    // unison-mcp から駆動不可（ハング）＋要 repo 起動のため follow-up（creo mem_1CcuR73WSNFkyAgzVJvWyF）。
    if let Err(e) = server
    ...
    // repo register → "process.up" / unregister・切断 → "process.down"。これで `vp events` が
    // emit なしでも「repo が上がった/落ちた」を最初から持つ（build/test 等の追加 source は follow-up）。
    {
                    ...
                    Ok(ProcessLifecycleEvent::Add {
                        ...
                        event_log
                            .emit(
                                "process.up",
                                Some(repo_name),
                                serde_json::json!({ "path": repo_path, "port": port, "pid": pid }),
                            )
                            .await;
                    }
    ...
    //   - `list`      : RPC、 現在の running_repos snapshot を JSON で返す
    ...
    if let Some(ref running_repos) = state.running_repos {
        let running_processes_snapshot = running_repos.clone();
        ...
        // sidebar と同じ repo 順 (repo_order) を引くため daemon_cap も capture。
        let daemon_cap_for_channel = state.daemon_cap.clone();
        server
            .register_channel("daemon-repo", {
                move |_ctx, stream| {
                    let running_repos = running_processes_snapshot.clone();
                    ...
                    async move {
                        ...
                        loop {
                            ...
                            match method.as_str() {
                                "list" => {
                                    let snapshot: Vec<ProcessSnapshot> = running_repos
                                        .read()
                                        .await
                                        .values()
                                ...
                                "list_all_lanes" => {
                                    // cross-project lane view: running_repos (port/name の SSOT)
                                    // と lane_registry を join し、repo ごとに lanes を束ねて返す。
                                    ...
                                    let repos =
                                        build_node_lanes(&running_repos, &lane_registry, &daemon_cap)
                                            .await;
                                    if channel
                                        .send_response(
                                            request_id,
                                        ...
                                        match rx.recv().await {
                                            Ok(event) => {
                                                let payload = match serde_json::to_value(&event) {
                                                    ...
                                                    Err(e) => {
                                                        tracing::warn!(
                                                            "daemon-repo event serialize 失敗: {}",
                                                            e
                                                        );
                                                        continue;
                                                    }
    ...
    // =========================================================================
    // "lanes" Channel（L0 SP-portless lanes slice — vp-app per-repo lane 購読の Daemon 集約版）
    // =========================================================================
    // 旧経路では vp-app が各 repo (:33000+) の "lanes" channel に直結して repo ごとの lane
    // snapshot を購読していた。 SP-portless 化により、 vp-app は Daemon :32000 の本 channel 1 本に
    // 集約する。
    //
    // 経路: repo register/lanes-diff (QUIC Push) → registry channel → lane_registry + lane_change_tx
    //       → 本 channel の subscriber → vp-app。
    //
    // プロトコル:
    //   1. client: open_channel("lanes") → request("subscribe", {"repo_path": "<dir>"})
    //   2. server: ack 応答後、 lane_change_tx を購読 → 当該 repo の現 snapshot を初期配信
    //      (`send_event("snapshot", LanesSnapshot)`)、 以降 lane_change のたび再 push。
    //
    // event 形は repo "lanes" channel と一致させ、 vp-app consumer を無改造に保つ (send_lanes_snapshot)。
    if let Some(ref lane_registry) = state.lane_registry {
        ...
        let running_repos = state.running_repos.clone();
        ...
        server
            .register_channel("lanes", {
                move |_ctx, stream| {
                    ...
                    let running_repos = running_repos.clone();
                    let vpdb = vpdb.clone();
                    async move {
                        let channel = UnisonChannel::new(stream);

                        // handshake: 全 repo 単位 channel 共通の subscribe ({repo_path}→path_key)。
                        // canvas / control / repo-proxy と同一 helper に統一 (bespoke 重複を排除、
                        ...
                        // 初期 snapshot 配信
                        if send_lanes_snapshot(&channel, &lane_registry, &path_key, &wiremsg_store, &running_repos, &vpdb)
                            .await
                            .is_err()
                        {
                            return Ok(()); // client 切断
                        ...
                        // 変更 push loop: 当該 repo の lane_change のたび現 snapshot を再配信
                        loop {
                            match rx.recv().await {
                                Ok(changed_key) => {
                                    if changed_key != path_key {
                                        continue; // 別 repo の変更は無視
                                    }
                                    if send_lanes_snapshot(&channel, &lane_registry, &path_key, &wiremsg_store, &running_repos, &vpdb)
                                        .await
                                        .is_err()
    ...
    // =========================================================================
    // "gui-ingest" Channel（repo → daemon の GUI-bound content push 受け口。旧名 "canvas-ingest"）
    // =========================================================================
    // doc 44 P1 (fold-in) 以前は、各 repo の pusher (`discovery::spawn_daemon_uplink`) が
    // board topic の RepoMessage をこの channel に push していた。fold-in 後は
    // repo の TopicRouter を daemon が直接購読するため、この受け口に来る push は無い
    // （`canvas_router_for` が repo 起動時に実 router へ差し替える）。channel 自体は
    // 外部からの ingest 口として残置（doc 52 §6: 対の配信 channel は "gui"）。
    //
    // プロトコル: repo が open_channel("gui-ingest") → request("subscribe", {repo_path}) →
    //   以降 send_event("pane", <RepoMessage JSON>) を流す。 daemon は route() するのみ (応答不要)。
    {
            ...
            .register_channel("gui-ingest", {
                move |_ctx, stream| {
                    ...
                    async move {
                        ...
                        // repo から push される board RepoMessage を router に route。
                        // event は method="pane"、 payload = RepoMessage JSON (repo の canvas
                        // channel と同形)。
                        loop {
                            let msg = match channel.recv().await {
                                Ok(m) => m,
                                Err(_) => break, // repo 切断 (canvas pusher が backoff 再接続する)
                            };
                            ...
                            match serde_json::from_value::<crate::protocol::RepoMessage>(value) {
                                Ok(pm) => router.route(pm).await,
                                Err(e) => {
                                    tracing::warn!(
                                        "canvas-ingest: RepoMessage decode 失敗 (key={}): {}",
                                        path_key,
                                        e
                                    );
                                }
    ...
    // `subscribe("repo/board/#")` し、 retained 初期配信 (最新 board 等) + live delta を
    // `send_event("pane", <RepoMessage JSON>)` 形で配る。
            ...
            .register_channel("gui", {
                move |_ctx, stream| {
                    ...
                    async move {
                        ...
                        // repo-proxy が実証済の並行 send/recv パターン)。
                        ...
                        // `repo/terminal/data/{lane}/out` を指定して demand を立てる。
                        ...
                        let pattern = pattern.unwrap_or_else(|| "repo/board/#".to_string());
                        ...
                        // 上り: surface → Daemon → repo control へ forward (S3 terminal_write/resize)。
    ...
    // =========================================================================
    // 旧: 各 repo が outbound で "control" channel を張り、daemon が UnisonChannel を
    // control_channels[path_key] に保持して process 操作を reverse-route していた。
    // repo プロセスが daemon に畳み込まれたため channel 自体が不要になり、同じ dispatch を
    // `RepoRuntimes::dispatch` が in-process で直接呼ぶ。
    // これに伴い「repo 未接続で無言破棄」「refire_active_demands の空振りレース」
    // 「高速再接続で旧 handler が新 channel を clobber」の 3 バグクラスが消滅した。

    // =========================================================================
    // "repo-proxy" Channel（L0 SP-portless control slice — 外部 client → Daemon → repo reverse）
    // =========================================================================
    // MCP/CLI は repo listen port ではなく本 channel に繋ぎ、 handshake {repo_path} 後に
    // process method (show/clear/tmux/process/wire 等) を request する。 daemon は当該 repo の
    // control channel を逆用して forward し、 応答を client に relay する。 repo "process" channel と
    // 同一 method・同一 dispatch (repo 側 `dispatch_repo_method`) なので、 client から見た挙動は
    // repo 直結と不変 (= repo portless 化しても透過)。
    {
        let control_channels = state.control_channels.clone();
        server
            .register_channel("repo-proxy", {
                move |_ctx, stream| {
                    let control_channels = control_channels.clone();
                    async move {
                        let channel = UnisonChannel::new(stream);
                        ...
                        loop {
                            ...
                            // 当該 repo の control channel を逆用して forward (= Daemon→repo reverse)。
                            let response = forward_to_sp_control(
                                &control_channels,
                                &path_key,
                                &method,
    ...
    // 変換して push する単機能 channel。 daemon-repo と違い method 分岐は無く、 接続 = 購読
            ...
            .register_channel("daemon-device", {
                move |_ctx, stream| {
                    ...
                    async move {
                        ...
                        // 購読に差し替えるか buffer_size を調整する (daemon-repo は lag を warn 可視化)。
                        let push_channel = channel.clone();
                        let pusher = tokio::spawn(async move {
    ...
    // =========================================================================
    // Registry Channel（稼働 repo の read-only 照会）
    // =========================================================================
    //
    // doc 44 P1 (fold-in) 以前は「repo 自己登録 — QUIC 永続接続による即時登録・即時死亡検出」
    // の control plane で、register / unregister / heartbeat / lanes/* を受けて daemon 側の
    // running_repos / lane_registry / process_presence を維持していた。
    //
    // fold-in で repo が daemon プロセス内に入り、自己登録しに来る repo が消滅した。
    // 上記 state の維持は起動元（`start_process` / `publish_lanes`）が直接行う daemon-canonical
    // に移ったため、repo 向け method 群と切断時の後始末は撤去した。
    //
    ...
    if let Some(ref running_repos) = state.running_repos {
        ...
        server
            .register_channel("registry", {
                move |_ctx, stream| {
                    // doc 44 P1 (fold-in): repo 向けの register / unregister / heartbeat /
                    // lanes/* は撤去した（自己登録しに来る repo プロセスが存在しない）。
                    // 残るのは read-only の `list` だけなので、依存も running_repos 1 本。
                    let running_repos = running_repos.clone();
                    async move {
                        let channel = UnisonChannel::new(stream);
                        ...
                        loop {
                            ...
                            match method.as_str() {
                                "list" => {
                                    let list =
                                        registry_process_snapshot(&running_repos).await;
                                    if channel
                                        .send_response(
                                            request_id,
                                            "list",
                        ...
                        // doc 44 P1 (fold-in): repo 切断時の自動除去は撤去した。
                        // `registered_name` を立てる `register` が無くなったため到達不能で、
                        // かつ repo の生存は Daemon 自身の `RepoRuntimes` が持つ
    ...
    // daemon control Channel（repos mutation: CLI → Daemon 直接 Unison）
    //
    // control plane 一元化: repos は daemon 権威 (db/machine) なので、 CLI は repo を経由せず
    // daemon に直接 Unison RPC する。 registry (repo 自己登録専用) とは責務を分離した
    ...
    server
        .register_channel("wire", {
            ...
            move |_ctx, stream| {
                let state = state.clone();
                async move {
                    // repo は永続 link の 1 channel に全 wire request を多重化する (daemon_wire.rs の
...
///
/// 観測された故障モード: HTTP 面は生存したまま QUIC accept が wedge し、 repo uplink /
/// wire / repo-proxy が全滅する「片肺死」。 `quic.start()` は club-unison 内で永久 block
...
/// (「プロセスは死ぬがコンテキストは蘇る」 — repo は setsid 分離で生存、 lane claude は --resume)。
///
...
mod tests {
    ...
    // =====================================================================
    // daemon control Channel — repos mutation dispatch (handle_daemon_control)
    //
    ...
    async fn daemon_control_add_list_remove() {
        ...
        // add_repo は path.is_dir() を要求するので実在 dir (temp_dir) を使う
    ...
    /// `repos/update` が rename + enabled を 1 往復で適用する（旧 HTTP の合成 update）。
    #[tokio::test]
    ...
    /// `repos/sync` は ghost 無しなら `{removed: []}` を返す（旧 HTTP と同じ形）。
    #[tokio::test]
    ...
    /// `registry.list` の snapshot が `RunningRepo` の Serialize をそのまま写すこと。
    ///
    /// vp-app の `list_processes`（sidebar の runtime port join + Activity の稼働数）が
    /// 依存する面。旧 HTTP route と同じ `running_repos` map / 同じ Serialize を通るので、
    ...
    async fn daemon_control_new_methods_require_their_fields() {
        ...
        for (method, payload) in [
            ("repos/update", serde_json::json!({})),
            ("repos/restart", serde_json::json!({})),
            ("repos/pointview", serde_json::json!({})),
    ...
    /// FSM 投影: wire payload からの関与 repo 抽出 (send = from + to[]、 ack = agent)
    #[test]
    fn collect_wire_repos_extracts_from_send_and_ack_payloads() {
        ...
        assert_eq!(
            collect_wire_repos(&send),
            vec!["creo-memories".to_string(), "vantage-point".to_string()],
            "from + to[] の repo を dedup して抽出 (BTreeSet = 辞書順)"
        );
        ...
        assert_eq!(collect_wire_repos(&ack), vec!["vantage-point".to_string()]);

        // address 無し / 不正形は空
        assert!(collect_wire_repos(&serde_json::json!({})).is_empty());
        assert!(
            collect_wire_repos(&serde_json::json!({"from": "bare-name"})).is_empty(),
            "@ 無しの address は repo を持たない"
        );
    }
}
