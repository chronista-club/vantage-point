...
//!
//! 旧実装は `reqwest::blocking` で `/api/daemon/projects/{sync,reload}` を叩いていた。
...
pub fn notify_daemon_sync() -> Option<SyncOutcome> {
    let removed = daemon_control_blocking(|client| async move { client.projects_sync().await })?;
...
pub fn notify_daemon_reload() {
    let _ = daemon_control_blocking(|client| async move { client.projects_reload().await });
}

/// 登録 project 一覧を daemon から取得する (`(name, path)` の組)。
///
/// doc 45 段 2: 旧 `GET /api/daemon/projects` の後継。daemon 不在は None
/// (caller は projects.kdl フォールバックに落とす)。
pub fn list_projects_blocking() -> Option<Vec<(String, String)>> {
    let projects = daemon_control_blocking(|client| async move { client.projects_list().await })?;
    Some(
        projects
            .iter()
            .filter_map(|p| {
...
/// 各要素は `{project_name, port, pid, project_path}`（fold-in 後は port=0 / pid=Daemon 自身なので
