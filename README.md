...
```
Vantage/
├── Package.swift           # ClaudeIntegrationパッケージ定義
├── Vantage.xcodeproj      # 統合プロジェクト
├── Sources/ClaudeIntegration/     # Claude Integration統合ライブラリ
├── Vantage/              # visionOSアプリソース
│   ├── AI/               # AIアシスタント機能
│   ├── Console/          # デバッグコンソール
│   ├── VantageMac/       # macOSテストアプリ
│   └── Shaders/          # Metalシェーダー
└── VantageMac/           # macOSテストアプリソース（移行中）
```
...
`VantageMac`はmacOS上でClaude APIの動作を確認するためのテストアプリです。

#### VantageMacターゲットの追加方法

1. Xcodeで`Vantage.xcodeproj`を開く
2. File → New → Target...
3. macOS → App を選択
4. 以下の設定で作成：
   - Product Name: VantageMac
   - Interface: SwiftUI
   - Language: Swift
5. 作成後、`Vantage/VantageMac/`内のファイルをターゲットに追加
