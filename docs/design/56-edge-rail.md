...
- **設定画面**（app 級・サイドバー下部・daemon status の上）= 別糸
  ...
  - **設計は [59-settings-page.md](./59-settings-page.md) へ**（2026-08-31）— 住所は上記のまま、
    項目 9 つが確定した。⚠️ **最初の設定項目は rail トグルではなくなった** — mako が選ばず
    （A 形が未実装 = トグルの片側が存在しないため）。rail トグルは設定基盤が立った後に足す
