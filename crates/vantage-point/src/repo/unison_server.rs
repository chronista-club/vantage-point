...
async fn handle_console_set_model(
...

    {
        ...
        match &model {
            Some(m) => crate::lane::engine_model::record(&addr.repo, &lane_label, m),
            None => crate::lane::engine_model::clear(&addr.repo, &lane_label),
        }
...
mod tests {
    ...
    async fn session_set_act_requires_session_and_switches_that_session() {
        ...
        assert_eq!(
            session_registry::root_act(&addr.repo, "root"),
            SessionAct::Chat,
