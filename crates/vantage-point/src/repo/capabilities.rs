...
impl RepoCapabilities {
    ...
    pub async fn new(config: CapabilityConfig) -> Self {
        ...
        // Agent Capability
        let mut agent = AgentCapability::new().with_working_dir(config.repo_dir.clone());
    ...
    pub fn start_event_bridge(
        &self,
        hub_sender: tokio::sync::broadcast::Sender<crate::protocol::RepoMessage>,
...
/// CapabilityEvent を RepoMessage に変換
///
/// 注: PR-α-2 (VP-112) で device 集約を daemon に移管したため、 repo (Repo) の
...
) -> Option<crate::protocol::RepoMessage> {
    ...
    match event.event_type.as_str() {
        ...
        "agent.text_chunk" => {
            let content = event
                ...
                .unwrap_or("")
                .to_string();
            Some(RepoMessage::ChatChunk {
                content,
                done: false,
            })
        }

        "agent.done" => Some(RepoMessage::ChatChunk {
            content: String::new(),
            done: true,
        }),
...
mod tests {
    ...
    async fn test_process_capabilities_new() {
        let config = CapabilityConfig {
            repo_dir: "/tmp/test".to_string(),
        };

        let _caps = RepoCapabilities::new(config).await;
    }
    ...
    async fn test_process_capabilities_initialize() {
        ...
        let caps = RepoCapabilities::new(config).await;
