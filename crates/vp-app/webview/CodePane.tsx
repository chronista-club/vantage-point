 ...
 * - 名札 = 開いている rel_path + `→ Board` 投擲 + ✕
 *
 * ## 配線（entry.tsx が結ぶ）
 *
 * - demand: pane open（`vp:code-view`）で `code:list`、file 選択で `code:read`、
 *   投擲で `code:board` を送る（⚠️ 3 tag とも app.rs `is_main_ipc_tag` の allowlist と対）
...
export interface CodePaneController {
	...
	/** 選択中（または開いている）file を board へ投擲（directive `p` / 名札ボタン）。 */
...
export function mountCodePane(host: HTMLElement): CodePaneController {
	...
	// pane の開閉（vp:code-view の写し）。⚠️ 投擲 guard 用 — 旧 picker の「非表示なら
	// no-op」を引き継ぐ（閉じたまま ⌘p で「最後の file」が飛ぶ驚きを作らない）。
	...
	const controller: CodePaneController = {
		setLane(address: string | null): void {
			...
			// paneOpen も一旦倒す — 直後の setActiveLane の dispatch（同期）が正しい値を書く。
			setPaneOpen(false);
			setEntries([]);
			setTruncated(false);
			setQuery("");
		...
		sendSelectedToBoard(): void {
			const addr = lane();
			if (addr === null || !paneOpen()) return;
			// 選択が file ならそれ、そうでなければ開いている file（右列）を投げる。
			const item = view()[selectedIndex()];
			const rel =
				item && item.entry.kind === "file"
					...
					: openedPath();
			if (!rel) return;
			sendIpc({ t: "code:board", lane: addr, rel_path: rel });
		},
	};
		...
		<div class="code-pane" onKeyDown={onKeyDown}>
			<div class="code-plate">
				...
				<button
					...
					title="選択 file を Board へ投擲（⌘ hold p）"
					onClick={() => controller.sendSelectedToBoard()}
				>
					→ Board
				</button>
				<button
