 ...
 *   接続し、 残り 4 関数 (renderError / handleAddWorkerResult / handleStandsResult /
...
declare global {
  interface Window {
    ...
    /** Rust → JS: Add Worker form の結果通知 (PR-3 で実装)。 */
    handleAddWorkerResult?: (msg: unknown) => void
...
export function installIpcBridge(): void {
  ...
  window.handleAddWorkerResult ??= (msg) =>
    console.debug('[vp-sidebar] handleAddWorkerResult (stub):', msg)
