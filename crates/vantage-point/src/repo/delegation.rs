...
//! 人間の介入なしで自走させる。**v1 はローカルのみ**（same Daemon / same repo / 2 lane）、federation
...
///
/// `requester` / `doer` は論理 wire address（federation 不変条件）。repo は `daemon_wire::call` で
/// daemon に proxy し、record は SurrealDB に永続化される（durable、repo 再起動を跨いで生存。
...
/// が解する lane address query（`<repo>/root` / `<repo>/performer/<name>`）に翻訳する。
///
...
/// - `agent@<repo>/<name>` → `<repo>/performer/<name>`
///
...
pub(crate) fn lane_query_for(addr: &str) -> String {
    ...
    match rest.split_once('/') {
        ...
        // `agent@<repo>/<name>` → performer lane。
        Some((repo, name)) => format!("{repo}/performer/{name}"),
        // `agent@<repo>` → conductor lane。
...
///
/// id 採番 → daemon 中央 store に persist（durable）→ doer を repo-local wake → delivered 記録。
...
pub(crate) async fn handle_delegate(
    ...
    // doer を repo-local で wake（resumable continuation を送る）。
...
pub(crate) async fn handle_complete(
    ...
    // requester を repo-local wake（NeedsInput は質問を、Done/Failed は outcome を届ける）。
...
//
// process 管理の reconcile と同じ DNA: push（repo-local wake）の取りこぼし / doer 沈黙を、
...
async fn reconcile_pulse(
    ...
    // tmux decoupling PR1: (path_key, lane) ペアで収集（forward 先 repo を path_key で特定）。
