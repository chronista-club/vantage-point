//! Stand Capability Integration
//!
//! Capability システムを Stand に統合するモジュール。
...
/// Stand Capability Manager
///
/// Stand で使用する全ての Capability を管理する。
pub struct StandCapabilities {
...
impl StandCapabilities {
    /// 新しい StandCapabilities を作成・初期化
    ...
    pub fn start_event_bridge(
        &self,
        hub_sender: tokio::sync::broadcast::Sender<crate::protocol::StandMessage>,
    ) -> tokio::task::JoinHandle<()> {
        let event_bus = self.event_bus.clone();

        tokio::spawn(async move {
            // EventBus を購読
            let mut subscription = event_bus.subscribe("stand-bridge", "*").await;

            while let Some(event) = subscription.recv().await {
                // CapabilityEvent を StandMessage に変換
                let stand_msg = capability_event_to_stand_message(&event);
...
/// CapabilityEvent を StandMessage に変換
fn capability_event_to_stand_message(
    event: &crate::capability::CapabilityEvent,
) -> crate::protocol::StandMessage {
    ...
    match event.event_type.as_str() {
        ...
        "agent.text_chunk" => {
            let content = event
                ...
                .unwrap_or("")
                .to_string();
            StandMessage::ChatChunk {
                content,
                done: false,
            }
        }

        "agent.done" => StandMessage::ChatChunk {
            content: String::new(),
            done: true,
        },
        ...
        // Capability 状態変更
        "capability.initialized" | "capability.state_changed" => StandMessage::DebugInfo {
            level: crate::protocol::DebugMode::Simple,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_stand_capabilities_new() {
        ...
        let caps = StandCapabilities::new(config).await;
        assert!(caps.midi.is_none());
        assert!(caps.bonjour.is_none());
    }

    #[tokio::test]
    async fn test_stand_capabilities_with_midi() {
        let config = CapabilityConfig {
            project_dir: "/tmp/test".to_string(),
    ...
    #[tokio::test]
    async fn test_stand_capabilities_with_bonjour() {
        let config = CapabilityConfig {
            project_dir: "/tmp/test".to_string(),
            midi_config: None,
    ...
    #[tokio::test]
    async fn test_stand_capabilities_initialize() {
        let config = CapabilityConfig {
            project_dir: "/tmp/test".to_string(),
            midi_config: None,
            bonjour_port: None,
