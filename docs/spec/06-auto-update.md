...
Conductor Process が更新を検知し、ユーザー確認後に自動更新・再起動を行う。
...
**概要**: Conductor Process が起動時に更新を確認する
...
**詳細**:
1. Conductor Process に停止リクエスト送信
2. 稼働中の Project Process を graceful shutdown
3. バイナリを更新
4. Conductor Process を再起動
...
- 起動前に Conductor Process が稼働していることを確認
- 稼働していなければ自動起動

**受け入れ条件**:
- [ ] `vp app` でアプリが起動する
- [ ] Conductor Process が自動起動する
- [ ] 既に起動中の場合はフォーカスを移動
...
```
┌─────────────────┐     ┌──────────────────┐
│ VantagePoint.app│◄───►│Conductor Process │
│   (Swift)       │     │  (vp conductor)  │
└────────┬────────┘     └────────┬─────────┘
         │                       │
         │  ┌────────────────────┴─────────────────────┐
...
sequenceDiagram
    ...
    participant Conductor as Conductor Process
...
- [05-capability.md](05-capability.md) - Capability 仕様
