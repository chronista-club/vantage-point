...
struct ContentViewWithConsole: View {
    ...
    var body: some View {
        mainView
            ...
            .alert("APIキーを入力", isPresented: $showingAPIKeyAlert) {
                ...
                Button("設定") {
                    Task {
                        await viewModel.setAPIKey(apiKey)
            ...
            .sheet(isPresented: $showingSessionList) {
                SessionListView(
                    sessionManager: viewModel.sessionService,
                    showingSessionList: $showingSessionList,
                    onSessionSelected: { sessionId in
                        viewModel.switchToSession(sessionId)
                        if let session = viewModel.sessionService.currentSession {
                            // messagesは読み取り専用なので直接代入できない
    ...
    private var mainView: some View {
        HSplitView {
            ...
            if showConsole {
                // コンソールビュー
                ConsoleView(logs: viewModel.consoleLogs, onClearLogs: {
                    viewModel.clearLogs()
                })
