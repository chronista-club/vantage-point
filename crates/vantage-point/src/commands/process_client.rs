...
//!
//! L0 portless: 旧 `ProcessClient` (SP HTTP 直結) は撤去。 CLI は Daemon :32000 の process-proxy ask
//! で SP を操作する。
//! - `daemon_process_request` / `_blocking`: daemon process-proxy ask の core (async / sync 版)。
//! - `daemon_lanes_snapshot_blocking`: Daemon "daemon-process" channel の cross-project lane 一覧。
...
// L0 portless: `ProcessClient` (SP HTTP 直結 blocking client) は撤去済。 CLI の Daemon ask は
// `daemon_process_request_blocking` / `resolve_project_path_from_target` を使う (SP port 解決も不要)。

/// F6 (doc 27 §3.4.5/§6): Daemon :32000 の "process-proxy" channel 経由で SP process method を
/// ask する helper。 L0 portless 後の CLI → SP 操作の唯一の入口 (旧来の SP 直結 port QUIC は撤去)。
/// daemon が project_path から path_key を正規化して当該 SP の
...
/// operation は [`daemon_process_request_with_timeout`] で個別に伸ばす。
pub async fn daemon_process_request(
    daemon_port: u16,
    project_path: &str,
    method: &str,
    payload: serde_json::Value,
) -> Result<serde_json::Value> {
    daemon_process_request_with_timeout(
        daemon_port,
        project_path,
        method,
        payload,
...
/// [`daemon_process_request`] の outer timeout を呼び出し側が指定する版。
///
...
pub async fn daemon_process_request_with_timeout(
    daemon_port: u16,
    project_path: &str,
    method: &str,
    payload: serde_json::Value,
    ...
    let work = async {
        ...
        if let Some(err) = resp.get("error").and_then(|v| v.as_str()) {
            bail!("daemon process-proxy error ({}): {}", method, err);
        }
    ...
    match tokio::time::timeout(timeout, work).await {
        Ok(result) => result,
        Err(_) => bail!(
            "Daemon ({}) process-proxy.{} が {:?} 以内に応答しませんでした",
            addr,
            method,
            timeout
        ),
...
/// Daemon "daemon-process" channel の `list_all_lanes` を 1 回 ask する (CLI 用 blocking)。
///
/// 応答は `{"projects":[{"project_name":..,"project_path":..,"lanes":[LaneInfo]}]}`
/// (`daemon::server::build_node_lanes`)。 project ごとに束ねた **cross-project の lane 一覧**で、
    ...
    rt.block_on(async {
        let work = async {
            ...
            let channel = client
                ...
                .await
                .map_err(|e| anyhow::anyhow!("open daemon-process channel 失敗: {}", e))?;
            let resp: serde_json::Value = channel
                .request::<serde_json::Value, serde_json::Value>(
                    "list_all_lanes",
                    &serde_json::json!({}),
                ...
                .await
                .map_err(|e| anyhow::anyhow!("daemon-process list_all_lanes 失敗: {}", e))?;
            if let Some(err) = resp.get("error").and_then(|v| v.as_str()) {
                bail!("daemon-process error (list_all_lanes): {}", err);
            }
            Ok::<serde_json::Value, anyhow::Error>(resp)
        };
        match tokio::time::timeout(std::time::Duration::from_secs(10), work).await {
            Ok(result) => result,
            Err(_) => bail!(
                "Daemon ({}) daemon-process.list_all_lanes が 10 秒以内に応答しませんでした",
                addr
            ),
        }
    })
...
/// 同期版 (CLI 用入口)。 専用 runtime で `daemon_process_request` を block_on する。
/// async context (flow.rs 等) からは runtime 内 runtime で panic するので `daemon_process_request`
/// を直接 await すること。
pub fn daemon_process_request_blocking(
    daemon_port: u16,
    project_path: &str,
    method: &str,
    payload: serde_json::Value,
    ...
    let rt = tokio::runtime::Runtime::new()
        .map_err(|e| anyhow::anyhow!("tokio runtime build failed: {}", e))?;
    rt.block_on(daemon_process_request(
        daemon_port,
        project_path,
        method,
        payload,
...
/// L0 portless: target → project_path（daemon process-proxy handshake の安定 identifier）。
///
