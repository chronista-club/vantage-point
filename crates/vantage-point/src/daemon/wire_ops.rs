...
//! 本 module は store 直結のロジック層 (`*_store` 関数) と、 daemon の "wire" channel
//! （`handle_wire_channel`）が呼ぶ [`dispatch_wire`] を提供する（旧 axum wrapper は doc 45 段 4 で撤去済）。
...
//!
//! 正規化 (bare `"agent"` → `"agent@<repo>"`) は repo 文脈を持つ repo の入口で
...
/// bare `"agent"` を reject する (daemon は repo 文脈が無く正規化できない)
fn validate_addr(addr: &str) -> Result<(), String> {
    if addr == "agent" {
        return Err(
            "wire: bare \"agent\" は中央 store では曖昧です。 repo (port 33000 台) 経由で送るか、 \
             qualified 形 (agent@<repo>) を指定してください"
                .to_string(),
        );
    }
    Ok(())
...
mod tests {
    ...
    /// bare "agent" は中央 store で reject される (正規化は repo の責務)
    #[tokio::test]
