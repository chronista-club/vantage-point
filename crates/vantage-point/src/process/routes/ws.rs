...
use crate::protocol::{
    BrowserMessage, ChatMessage, ChatRole, ComponentAction, DebugMode, HistoryMessage,
    ProcessMessage,
};
                    ...
                    if let Ok(browser_msg) = serde_json::from_str::<BrowserMessage>(&text) {
                        match browser_msg {
                            ...
                            BrowserMessage::ResetSession => {
                                ...
                                // Notify browser
                                hub.broadcast(ProcessMessage::ChatMessage {
                                    message: ChatMessage {
                            ...
                            BrowserMessage::SwitchSession { session_id } => {
                                ...
                                if let Some(entry) = mgr.switch_to(&session_id) {
                                    ...
                                    hub.broadcast(ProcessMessage::SessionSwitched {
                                        session_id: session_id.clone(),
                                        name,
                                    });

                                    // Send session history for UI restoration
                                    hub.broadcast(ProcessMessage::SessionHistory {
                                        session_id: session_id.clone(),
                                        messages,
                                    });
                            ...
                            BrowserMessage::CloseSession { session_id } => {
                                ...
                                if mgr.close(&session_id) {
                                    hub.broadcast(ProcessMessage::SessionClosed { session_id });
                                    // Send updated list
                                    hub.broadcast(ProcessMessage::SessionList {
                                        sessions: mgr.list(),
                                        active_id: mgr.active_id.clone(),
