...
mod tests {
    ...
    fn test_parse_and_as_str_roundtrip() {
        let path = TopicPath::parse("repo/board/command/show");
        assert_eq!(path.as_str(), "repo/board/command/show");
    ...
    fn test_parse_with_leading_trailing_slashes() {
        ...
        let path = TopicPath::parse("/repo/debug/log/");
        assert_eq!(path.as_str(), "repo/debug/log");
    ...
    fn test_is_retained_state() {
        let path = TopicPath::parse("repo/terminal/state/ready");
    ...
    fn test_is_not_retained_data() {
        let path = TopicPath::parse("repo/terminal/data/output");
        assert!(!path.is_retained());
    }
    ...
    fn test_display() {
        let path = TopicPath::parse("repo/debug/log");
        assert_eq!(format!("{}", path), "repo/debug/log");
    }

    #[test]
    fn test_from_str() {
        let path: TopicPath = "repo/debug/trace".into();
        assert_eq!(path.as_str(), "repo/debug/trace");
    }
    ...
    fn test_pattern_parse_literals() {
        let pattern = TopicPattern::parse("repo/board/command/show");
        assert_eq!(pattern.segments().len(), 4);
        assert!(matches!(&pattern.segments()[0], Segment::Literal(s) if s == "repo"));
    ...
    fn test_match_exact_literal() {
        let topic = TopicPath::parse("repo/board/command/show");
        let pattern = TopicPattern::parse("repo/board/command/show");
    ...
    fn test_no_match_different_literal() {
        let topic = TopicPath::parse("repo/board/command/show");
        let pattern = TopicPattern::parse("repo/board/command/clear");
        assert!(!topic.matches(&pattern));
    }
    ...
    fn test_match_multi_wildcard_zero_segments() {
        // `#` は0セグメントにも一致
        let topic = TopicPath::parse("repo/board/command");
        let pattern = TopicPattern::parse("repo/board/command/#");
        assert!(topic.matches(&pattern));
    }
    ...
    fn test_no_match_too_short() {
        let topic = TopicPath::parse("repo/board");
        let pattern = TopicPattern::parse("repo/board/command/show");
        assert!(!topic.matches(&pattern));
    }
    ...
    fn test_match_all_with_hash() {
        // `#` だけで全トピックに一致
        let topic = TopicPath::parse("repo/debug/log");
    ...
    fn test_pattern_display() {
        let pattern = TopicPattern::parse("repo/+/state/#");
        assert_eq!(format!("{}", pattern), "repo/+/state/#");
    }

    #[test]
    fn test_pattern_from_str() {
        let pattern: TopicPattern = "repo/debug/#".into();
        let topic = TopicPath::parse("repo/debug/log");
        assert!(topic.matches(&pattern));
    }
}
