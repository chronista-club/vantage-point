...
**Status**: **一周完了（2026-07-24）** — 11 項目すべて審査済。実装フェーズは別途設計。「1 枚の Echoes」を原点に、これまでの開発を
...
| 11 | machine を跨ぐ | hub federation | **§11 済** |
...
## 11. machine を跨ぐ（hub federation）— 2026-07-24 結論

### どうしたいか（mako）

> 疎通はしたんだけど、VP の改修してると止まったりして、まだまともに動かせてない。
> **まずはきちんと使えるようにするところ。** あるとすれば **一度 auth login したら、
> logout するまでそのまま使いたい**。
...
- 疎通優先で作ったのは正しい順。ただし auth の更新経路が細い（refresh token 不在 /
  #660 の refresh は reactive のみ / 失効 token を掴んだ daemon は login 後も読み直さない —
  既知バグ台帳 hub-federation-token-expiry と一致）が「何度も login」の根

### 何が必要か

- **auth の永続化**: refresh token の取得 + proactive refresh + daemon の credential
  hot-reload（login 後に daemon 再起動不要）。「一度 login したら logout まで」を契約に
- 改修（daemon 再起動）に耐える再接続の堅牢化。理想の絵はその後に描く

---

## 12. 一周の完了（2026-07-24）

11 項目すべて閉じた。実装の順序と切り方は本 doc を土台に別途設計する
（速く作ることを優先しない — 規範を維持）。
