...
//!
//! 旧実装は「稼働 repo を Daemon registry から列挙 → 1 本ずつ API 停止 → daemon 停止 →
//! daemon 起動 → 1 本ずつ detached spawn」という多段の段取りだったが、
//! 対象である repo プロセスが存在しなくなったため段取りごと不要になった。
//!
//! # ⚠️ 復元の権威が変わった（意味論の転換）
//!
//! 旧: **その時点で running だったもの**を記憶して戻す（実体ベース）
//! 新: 起動時 autostart が **`enabled` な repo** を起こす（設定ベース）
//!
//! 実挙動が変わる組み合わせが 2 つある:
//!
//! - `enabled=false` だが手動で起動していた repo は、**再起動後に戻らない**
//!   （`vp repos start` は enabled を見ないので、この状態は作れる）
//! - `enabled=true` だが手動で `vp repos stop` していた repo は、
//!   **勝手に生き返る**
//!
//! これは `vp daemon restart` と同じ挙動に揃った、とも言える（fold-in 後は
//! repo が daemon の中にいる以上、daemon の再起動は必ず autostart を通る）。
//! 「今動いているもの」ではなく「動かすつもりのもの」を権威にする整理で、
//! 停止を永続させたいなら `vp repos disable` を使う。
...
pub fn execute() -> Result<()> {
    ...
    println!();
    println!("repo は autostart で順次立ち上がります（`vp ps` で確認）");
    Ok(())
}
