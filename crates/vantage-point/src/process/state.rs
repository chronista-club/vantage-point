...
pub(crate) struct AppState {
    ...
    /// daemon mode では空文字列 (= cross-process forward は SP mode 専用)。
    ...
    pub daemon: Option<Arc<RwLock<ProcessManagerCapability>>>,
    ...
    /// 遷移ごとに更新する。SP / test mode では `Disabled` のまま（federation は daemon のみ）。
    ...
    /// daemon mode でのみ DeliveryActor が待ち受ける。 SP では未使用 (proxy が daemon に送るだけ)。
    ...
    /// project の lanes publish task (`publish_lanes`) が subscribe して daemon の集約 view を
    ...
    ///
    /// daemon mode (`run_daemon`) でのみ Some、 SP mode (`run`) では None。
    ...
    ///
    /// SP mode (`run`) でのみ Some、 daemon mode (`run_daemon`) では None。
    ...
    ///
    /// **daemon mode (`run_daemon`) でのみ Some**、SP mode (`run`) では None。delegation record は
    /// wire と同じく daemon の SurrealDB に中央化 (durable、SP 再起動を跨いで生存、Daemon
    /// reconcile loop の駆動源)。SP の `handle_delegate` 等は `daemon_wire::call("/api/delegation/*")`
...
pub(crate) async fn build_test_app_state(
    daemon: Option<Arc<RwLock<ProcessManagerCapability>>>,
) -> Arc<AppState> {
...
pub(crate) async fn build_test_app_state_with(
    ...
    Arc::new(AppState {
        ...
        // test fixture は SP 相当 (Daemon store 無し)。delegation の store test は
