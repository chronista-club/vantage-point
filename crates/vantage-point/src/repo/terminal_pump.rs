...
//! (`repo/terminal/data/{lane}/out`) に `LaneTerminalOutput` として route する。
...
/// 撃ち直される経路 (vp-app WS 瞬断の 1→0→1 再 subscribe / repo↔daemon control 再接続時の
...
/// 流す pump を spawn。
///
/// - `lane`: LaneAddress の Display 形 (`"vp/root"` / `"vp/sub/foo"`)。 vp-app が
...
/// - `topic_router`: repo の topic_router。 route 先 (daemon へは ingest が転送する)。
///
...
pub fn lane_topic(lane: &str) -> String {
    format!("repo/terminal/data/{}/out", lane.replace('/', "~"))
}
...
mod tests {
    ...
    async fn test_pump_routes_to_per_lane_topic() {
        ...
        let (_id, mut srx) = router.subscribe("repo/terminal/data/vp~root/out").await;
        ...
        let (topic, msg) = tokio::time::timeout(Duration::from_secs(1), srx.recv())
            ...
            .expect("timeout")
            .expect("recv");
        assert_eq!(topic, "repo/terminal/data/vp~root/out");
        match msg {
    ...
    async fn test_pump_replays_before_live() {
        ...
        for _ in 0..2 {
            let (_topic, msg) = tokio::time::timeout(Duration::from_secs(1), srx.recv())
                ...
                .expect("timeout")
                .expect("recv");
            if let RepoMessage::LaneTerminalOutput { data, .. } = msg {
                received.push(
    ...
    async fn test_pump_skips_empty() {
        ...
        let (_topic, msg) = tokio::time::timeout(Duration::from_secs(1), srx.recv())
            ...
            .expect("recv");
        match msg {
            RepoMessage::LaneTerminalOutput { data, .. } => {
                let decoded = base64::engine::general_purpose::STANDARD
                    .decode(data)
                    .expect("base64");
