...
> **対象 Issue**: `VP-165` — ポート割当が project リスト変更で不安定 + Whitesnake が port-keyed → 永続 msg がプロジェクト跨ぎで継承される / from 汚染
> **親 Epic**: `VP-156` — Msgbox routing 統一 + 永続化 first-class
...
- Linear: `VP-165`（本設計の対象）/ `VP-156` epic / `VP-164` / `VP-163`（発見元）/ `VP-147` / `VP-155`（SP spawn 経路）
