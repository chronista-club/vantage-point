...
pub struct LayoutGetParams {
    ...
    #[schemars(
        description = "Layout scope: 'gallery' (default; the component gallery mode) or 'app' (the main workspace panes: echoes=console, runner, devices, preview). Lane scopes are not exposed yet."
    )]
...
pub struct LayoutSetParams {
    /// 対象 scope（省略 = "gallery"、後方互換）
    #[schemars(
        description = "Layout scope: 'gallery' (default; the component gallery mode, applied with a spring transition) or 'app' (the main workspace panes: echoes=console, runner, devices, preview; applied as a jump smoothed by CSS transition). Lane scopes are not exposed yet."
    )]
    pub scope: Option<String>,
