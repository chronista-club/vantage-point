...
graph TD
    ...
    subgraph "Service Abstraction Layer"
        C[ClaudeServiceProtocol]
    end
    
    subgraph "Platform Implementation Layer"
        D[ClaudeCodeService<br/>macOS]
        E[ClaudeAPIService<br/>visionOS]
    end
...
```mermaid
flowchart TD
    A[createDefault] --> B{Platform Check}
    B -->|macOS| C[Try Claude Code]
    B -->|visionOS| D[Use API Service]
    C --> E{Claude Code Available?}
    E -->|Yes| F[ClaudeCodeService]
    E -->|No| G[Fallback to API]
    G --> D
    D --> H[ClaudeAPIService]
