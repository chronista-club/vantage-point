...
- 2026-09-13: v0.69.0 出荷後、実効権限表示と Plan / 通常モード切替に着手。
  CLI 0.154.0 の隔離スレッドで `thread/settings/update` による両方向の変更と
  `thread/settings/updated` の実効値を確認。モード変更は approval / sandbox を変更しない。
  変更なしの update は通知を返さないため、読み取り API の代用にしない。
  起動・再開応答に mode がなければ未確認として扱い、既定値を推測しない。
  UI は実効権限の要約と詳細を表示し、モード選択は native 通知で確定する。
  実行中・待機入力あり・設定操作中の切替は拒否し、権限 profile の変更は提供しない。
  モード候補の照会・変更 ACK・実効値通知は合計 25 秒の期限を共有する（daemon の応答期限は 30 秒）。
  期限を過ぎてから次の変更 RPC を送らず、書込未完了時は Queue と同じ接続終了処理を使う。
  操作中の通常送信も拒否し、会話を再起動して再送する経路には入れない。
  Team B の再レビューは PASS。VP Chat での表示・両方向の切替は実機確認待ち。

- 2026-09-13: app:swap と daemon 再起動後の VP Chat 実機で Queue を検収。
  B の待機表示とターン終了後の自動開始、C の編集後本文の受信、D の順序変更・取消を確認した。
  E は停止 → Chat / Console 往復後も一覧に残り、「再開」から実行できたとユーザーが確認。
  実行中の Enter による補足受信も確認済み。通常送信した最初の A は Queue の検収結果に含めない。
  編集中も自動開始を待たず、順番が来た時点の保存済み本文を実行する仕様はユーザー了承済み。
...
- 2026-09-13: Enter の steer と native Queue の追加・編集・取消・並べ替え・明示再開を実装。
  ...
  修正後の workspace テストは成功。この時点では VP Chat の Queue 実機確認は未完了だった。
...
- Queue の事前実測（Codex CLI 0.154.0、隔離 thread / 実 app-server）:
  ...
  実測用の使い捨てスクリプトは TDD 対象外。この時点では製品の VP Chat での Queue 実機検収は未完了だった。
...
自動レビューはユーザーの明示選択だけで有効にする。実装済み、実機確認待ち。
