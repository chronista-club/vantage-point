//! daemon ↔ vp-app の wire 型（`RepoInfo` / `RunningRepo` / `LaneInfo` / `LaneSessionsWire` …）。
//!
//! transport 非依存の共有型で、読み手は Unison client（`daemon::control`）/ QUIC 購読 / sidebar push の 3 者。
//! crate root に置く共有型のひとつ（doc 60 §2）。webview との wire 型は `generated/`、
//! HTTP `/api/health` の probe は `daemon::health_probe`（旧 `client.rs` を 6-0b で分解、2026-09-08）。
//! server 側の構造体の手書き mirror なので、形の差は decode test で固定する（doc 45）。
...
//   本 module は consumer として use で bring-into-scope する。
