...
#### 6.5.1.2 完結 — World A は消滅した（2026-07-26、#920〜#923）

| PR | 中身 |
|---|---|
| **#920** | xterm を vendored `<script>`（932KB を HTML に `include_str!`）→ **npm 依存**。正味 −390KB（不要判定済みの ImageAddon 79KB も撤去）。凍結解除の記録は §6.5.1.1 |
| **#921** | **本体の畳み込み** — inline JS 976 行 → `webview/term.ts`（xterm 配線）+ `webview/active-pane.ts`（pane 切替 + slot rect）。`main_area.rs` 1807 → 738 行 |
| **#922** | console の cols が PTY とズレる（zsh `PROMPT_EOL_MARK` = `%` の正体）を **level 駆動**で根治 |
| **#923** | `setActivePane` の **2 段 wrap 解消**。World A 由来の順序依存（wrap の install を DOMContentLoaded まで遅らせる必要）も消滅 |

**§6.5 が「reconcile はそこに触れていない」と書いた最大タイのクラスタ（4 件）は、これで構造的に
消えた** — 同じ概念を 2 言語で表現する状態も、片方だけが改修に追随する状態も、原理的に起きない。

##### 分割の判断が効いた

**#921 は「純粋な移設」に絞り、wrap 解消を #923 に分けた**。976 行の移設に挙動変更を重ねると
**回帰が出たとき原因を移設と変更のどちらにも帰せなくなる**ため。結果:
...
> （観測 1 回 → 機構の推定 → doc 化）。**症状が実在しても、再現が取れないうちは「原因」を
> 書かない**。書くなら「容疑者」と明示する。
