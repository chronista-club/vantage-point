...
public actor ClaudeClient {
    ...
    private func performRequest(_ claudeRequest: ClaudeRequest) async throws -> ClaudeResponse {
        ...
        if httpResponse.statusCode.isSuccessHTTPCode {
            ...
            if let error = try? decoder.decode(ClaudeError.self, from: data) {
                let apiError: ClaudeIntegrationError
                switch httpResponse.statusCode {
                case 429:
                    let retryAfter = httpResponse.value(forHTTPHeaderField: "Retry-After")
                        .flatMap { Double($0) }
                    apiError = ClaudeIntegrationError.rateLimited(retryAfter: retryAfter)
                case 400:
                    apiError = ClaudeIntegrationError.invalidRequest(error.message)
                case 500...599:
                    apiError = ClaudeIntegrationError.serverError(error.message)
                default:
                    apiError = ClaudeIntegrationError.httpError(
                        statusCode: httpResponse.statusCode,
                ...
                let apiError = ClaudeIntegrationError.httpError(
                    statusCode: httpResponse.statusCode,
    ...
    private func performStreamingRequest(_ claudeRequest: ClaudeRequest) async throws -> AsyncThrowingStream<StreamEvent, Error> {
        ...
        guard httpResponse.statusCode.isSuccessHTTPCode else {
            throw ClaudeIntegrationError.httpError(
                statusCode: httpResponse.statusCode,
