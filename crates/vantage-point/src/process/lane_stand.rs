...
//!   本 trait 名は名前空間競合なく再利用可能

use std::any::Any;
use std::collections::HashMap;
use std::sync::Arc;

/// Lane に host される Stand の **minimal marker trait** (PR-δ-1)。
///
...
pub trait LaneStand: Any + Send + Sync + 'static {
    ...
    /// `Arc<dyn LaneStand>::as_any()` 経由で `&dyn Any` を取得し `downcast_ref::<T>()` する。
    /// trait 自体に `Any` 制約があるが、 `&dyn LaneStand` から `&dyn Any` への自動 cast は
...
///
/// `HashMap<&'static str, Arc<dyn LaneStand>>` で **N 個 host** する。 key は
/// `LaneStand::stand_kind()` の戻り値 (= stand id)。 Arc-shared なので caller が typed
...
pub struct LaneStandRegistry {
    stands: HashMap<&'static str, Arc<dyn LaneStand>>,
}
...
impl LaneStandRegistry {
    ...
    ///
    /// `Arc<T>` を受け取り、 unsized coercion で `Arc<dyn LaneStand>` に変換して
    /// HashMap に格納。 caller は `Arc<T>` を呼び出し側に保持しても良い (= shared
    /// reference として複数 path で活用可能、 PR-ε で creo memory feed inject 等)。
    ///
    /// ## type bound 注意
    ///
    /// `T: LaneStand + ?Sized` ではなく `T: LaneStand` (sized) のみを許容。
    /// Sized 制約は `Arc<T> -> Arc<dyn LaneStand>` の unsized coercion を成立させるため必須
    /// (Rust の `CoerceUnsized<Arc<U>> for Arc<T>` は `T: Unsize<U>` 要求、 これは Sized T で満たされる)。
    pub fn insert<T: LaneStand>(&mut self, stand: Arc<T>) {
        let kind = stand.stand_kind();
        let dyn_stand: Arc<dyn LaneStand> = stand;
    ...
    /// `&*arc` で `&dyn LaneStand` deref する。
    pub fn get(&self, kind: &str) -> Option<&Arc<dyn LaneStand>> {
        self.stands.get(kind)
    }

    /// `kind` ID で specific Stand 型に downcast 取得。
    ///
    /// `Arc<dyn LaneStand>` → `&dyn Any` → `&T` の 2 段 cast。 失敗時 (`kind` 不在 / 型不一致)
    /// は `None`。 PR-δ-2 で PP caller (例: `routes/canvas.rs`) が
    /// `registry.get_typed::<PaisleyParkStand>("paisley_park")` で typed access する。
    pub fn get_typed<T: LaneStand>(&self, kind: &str) -> Option<&T> {
        self.stands.get(kind)?.as_any().downcast_ref::<T>()
    }

    /// `kind` ID の Stand を削除。 削除成功で `Some(Arc<dyn LaneStand>)`、 不在で `None`。
    pub fn remove(&mut self, kind: &str) -> Option<Arc<dyn LaneStand>> {
...
mod tests {
    ...
    fn registry_insert_arc_coercion_works() {
        // Arc<T> から Arc<dyn LaneStand> への unsized coercion が成立すること
    ...
    fn registry_get_returns_arc_dyn() {
        // get(kind) は Arc<dyn LaneStand> reference を返す、 stand_kind() で id 確認
        let mut reg = LaneStandRegistry::new();
