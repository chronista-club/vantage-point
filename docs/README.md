...
> **状態ラベル**: `確定` 現行仕様 / `実装中` コードに具体化進行中 / `検討` 採否未確定だが議論進行中 / `草案` 概念検討中・実装未着手。確定後に `VP-DESIGN-NNN` を採番。
>
...
| ID | 状態 | ドキュメント | 内容 |
|----|------|-------------|------|
| VP-DESIGN-001 | 確定 | [01-architecture.md](design/01-architecture.md) | システムアーキテクチャ |
| VP-DESIGN-002 | 確定 | [02-capability-evolution.md](design/02-capability-evolution.md) | Capability 進化システム |
