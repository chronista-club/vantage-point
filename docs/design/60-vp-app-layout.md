...
session file（`session.json` / `session.<N>.json`、instance ごと）に何が・いつ・どの順で書かれるかの表。
6-2a までは `session_state.save()` が 8 箇所に散っていた。6-2b b-1 で `Persist` に集め、この表を契約にする。

| 値 | 誰が正か | boot 窓（未取得時）に何を保持するか | いつ保存するか | RPC 失敗時 | primary / secondary |
|---|---|---|---|---|---|
