...
pub async fn start_unison_server(state: Arc<AppState>, http_port: u16) {
    ...
    // --- show ハンドラー ---
    {
        ...
        server
            .register_handler("show", move |payload| {
    ...
    // --- clear ハンドラー ---
    {
        let state = state.clone();
        server
            .register_handler("clear", move |payload| {
                let req: ClearRequest = serde_json::from_value(payload).map_err(|e| {
                    NetworkError::Protocol(format!("Invalid clear payload: {}", e))
                })?;
    ...
    // --- toggle_pane ハンドラー ---
    {
        let state = state.clone();
        server
            .register_handler("toggle_pane", move |payload| {
    ...
    // --- split_pane ハンドラー ---
    {
        let state = state.clone();
        server
            .register_handler("split_pane", move |payload| {
    ...
    // --- close_pane ハンドラー ---
    {
        let state = state.clone();
        server
            .register_handler("close_pane", move |payload| {
    ...
    // --- canvas.open ハンドラー ---
    {
        let state = state.clone();
        server
            .register_handler("canvas.open", move |_payload| {
    ...
    // --- canvas.close ハンドラー ---
    {
        let state = state.clone();
        server
            .register_handler("canvas.close", move |_payload| {
                let handle = tokio::runtime::Handle::current();
                handle.block_on(async {
                    let mut pid_guard = state.canvas_pid.lock().await;
