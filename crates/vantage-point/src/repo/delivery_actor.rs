...
//! - **lane Running + root mode=Chat → engine 直接注入 (チャネル E、 doc 34 §3)**。
...
//!
//! ## チャネル C の送出は repo control channel 経由 (tmux decoupling PR1)
//!
//! daemon は PtySlot を持たない（PtySlot は repo scope）ので、 lane を所有する repo の control
//! channel に `lane_nudge` を forward し、 repo 側で `LanePool::write_nudge` が PtySlot に直書きする。
//! 旧実装は lane registry の tmux session 名に daemon から直接 `send-keys` していた（cross-process
//! IPC namespace = tmux session への依存）が、 PR1 で repo-proxy に寄せて tmux 非依存化した。
...
/// - `agent@<repo>` → `<repo>/root`（lane 名省略 = 開発起点）
/// - `agent@<repo>/<name>` → `<repo>/<name>`
...
pub(crate) fn wire_agent_to_lane_display(addr: &str) -> Option<String> {
    ...
    match rest.split_once('/') {
        ...
        Some((repo, name)) if !repo.is_empty() && !name.is_empty() => {
            Some(LaneAddress::new(repo, name).to_string())
        }
        Some(_) => None,
    }
}
...
/// 所有 repo への forward に必要な `path_key`（= `control_channels` の key）と、 repo 側 `lane_nudge`
/// の宛先になる `lane_display`（`LaneAddress` の Display 形）を持つ。
#[derive(Debug, PartialEq)]
pub(crate) struct NudgeTarget {
    /// lane を所有する repo の path_key（`lane_registry` / `control_channels` の HashMap key）
    pub path_key: String,
...
/// forward する repo control method（channel 判別の純関数、doc 34 §3）。
///
/// 入力は **root session の mode**（doc 53 R1 — 旧 `LaneInfo.console_mode` 投影の廃止に伴い、
...
/// `(path_key, LaneInfo)` ペアにするのは、 forward 先 repo を `path_key` で特定するため
...
fn recipient_readiness(
    lane_nudgeable: bool,
    activity: Option<Option<crate::repo::cc_activity::CcState>>,
) -> Readiness {
    use crate::repo::cc_activity::CcState;
    if !lane_nudgeable {
...
/// wire agent address → headless dispatch 用の (VP_REPO, VP_LANE) (純関数、 R3-c)
///
...
async fn pulse(
    ...
    // tmux decoupling PR1: (path_key, lane) ペアで収集する。 path_key は所有 repo の control
    ...
    let activity = crate::repo::cc_activity::poll_cc_activity().await;
    let now = Instant::now();
    ...
    for (msg, agents) in &pending {
        for agent in agents {
            ...
            let act_view = activity.as_ref().map(|s| match &target {
                Some(t) => crate::repo::cc_activity::state_for_cwd(s, &t.cwd),
                None => None,
            });
            ...
            //
            // doc 53 R1: 配送 method の分水嶺は root session の mode = intent（registry 直読）。
            // pulse ごとに現在値を読む = 代表の mode が変わっても次 pulse で正しい経路に乗る。
            let root_mode = lane_identity_from_agent(agent)
...
///
/// lane と同じ identity env (`VP_REPO/VP_LANE` — doc 40 PR-3 で VP_CWD/VP_SESSION は退役) を渡し、
/// 当該 lane の wire address で wire_recv/wire_ack できるようにする (MCP server は
/// repo/user settings から解決され、 interactive lane と同経路)。
///
...
fn spawn_bg_dispatch(cwd: String, repo: String, lane: String, args: Vec<String>) {
    tokio::spawn(async move {
        ...
        match cmd.status().await {
            Ok(st) => tracing::info!(
                "wire delivery: bg dispatch 完了 (repo={}, lane={}, status={})",
                repo,
                lane,
                st
            ),
            Err(e) => tracing::warn!(
                "wire delivery: bg dispatch 子プロセス失敗 (repo={}, lane={}): {}",
                repo,
                lane,
                e
            ),
...
mod tests {
    ...
    fn nudge_method_follows_root_mode() {
        ...
        assert_eq!(
            nudge_method_for(SessionMode::Tui),
            "lane_nudge",
            "Tui は従来の PtySlot 直書き"
        );
        assert_eq!(
            nudge_method_for(SessionMode::default()),
            "lane_nudge",
